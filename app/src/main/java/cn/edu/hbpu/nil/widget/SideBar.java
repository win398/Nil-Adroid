...
public class SideBar extends TextView {
    ...
    private void init(AttributeSet attrs) {
        ...
        bigTextPaint.setColor(getResources().getColor(R.color.index_big));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        //不允许父控件拦截事件，那么事件在父控件中不起作用，而只到本控件中来
        ...
        if (y != 0) {
            for (int i = 0; i < letters.length; i++) {
                ...
                if (y >= currentItemY && y < nextItemY) {
                    ...
                    canvas.drawText(letters[i], w - getPaddingRight() - scaleWidth - bigTextSize, singleTextH + itemH * 13, bigTextPaint);
                }
            }
        }
